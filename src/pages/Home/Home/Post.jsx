...
const Post = ({ post, setResetCount }) => {
  ...
  const handleLike = () => {
    ...
    axios
      .post("http://localhost:5000/posts/like", payload)
      .then((res) => {
        setIsLiked(!isLiked);
  ...
  return (
    <div className="w-full bg-white shadow-md rounded-lg overflow-hidden py-2 my-5 pb-4">
          ...
          <div className="mt-2  flex justify-center bg-[#f7f7f6] rounded-lg p-2 w-full">
            <div className="w-full">
              {displayedImages.map((url, index) => (
                <div
                  key={index}
                  className="w-full overflow-hidden rounded-lg flex justify-center"
                >
                  {renderMedia(url)}
                </div>
              ))}
      ...
      <div className="flex items-center">
        ...
        <span className="text-sm text-gray-500">
          {isLiked && likeCount === 1
            ...
            : `${likeCount} ${
                likeCount === 1 ? "person has" : "people have"
      ...
      <div className="flex justify-between items-center mb-4 mx-5">
        <button
          className="btn w-[47%] flex gap-2 text-xl items-center"
          onClick={handleLike}
        >
          {isLiked ? (
        ...
        <button
          onClick={() => setIsCmtOper(!isCmtOper)}
          className="btn w-[47%] flex gap-2 text-xl items-center"
        >
